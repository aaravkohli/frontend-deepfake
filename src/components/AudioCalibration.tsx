...
import { AudioCalibrationConfig } from '../types/api';

interface AudioCalibrationProps {
  className?: string;
  onConfigChange?: (config: AudioCalibrationConfig) => void;
...
export default function AudioCalibration({
  ...
  const handleConfigUpdate = (updates: Partial<AudioCalibrationConfig>) => {
  ...
  const applyPreset = (type: 'threshold' | 'uncertainty', value: number) => {
